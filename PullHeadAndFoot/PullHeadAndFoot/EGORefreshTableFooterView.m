...
- (void)setState:(EGOPushRefreshState)aState{
	...
	switch (aState) {
		...
		case EGOOPushRefreshNormal:
			...
			if (_state == EGOOPushRefreshPushing) {
				[CATransaction begin];
				[CATransaction setAnimationDuration:FLIP_ANIMATION_DURATION];
...
- (void)egoRefreshScrollViewDidScroll:(UIScrollView *)scrollView {	
	...
	if (_state == EGOOPushRefreshLoading) {
...
//		offset = MIN(offset, 60);
		...
		if (_state == EGOOPushRefreshPushing && scrollView.contentOffset.y + (scrollView.frame.size.height) < scrollView.contentSize.height + RefreshViewHight && scrollView.contentOffset.y > 0.0f && !_loading) {
			[self setState:EGOOPushRefreshNormal];
		} else if (_state == EGOOPushRefreshNormal && scrollView.contentOffset.y + (scrollView.frame.size.height) > scrollView.contentSize.height + RefreshViewHight  && !_loading) {
			[self setState:EGOOPushRefreshPushing];
		}
...
- (void)egoRefreshScrollViewDidEndDragging:(UIScrollView *)scrollView {
	...
	if (scrollView.contentOffset.y + (scrollView.frame.size.height) > scrollView.contentSize.height + RefreshViewHight && !_loading) {
		...
		[self setState:EGOOPushRefreshLoading];
